...
class main:
    def __init__(self, default_font, appdata_dir, name):
        ...
        self.status_list = ["name", "job", "STR", "CON", "POW", "DEX", "APP", "SIZ", "INT",
                            "EDU", "年収", "財産", "SAN", "幸運", "アイデア", "知識", "耐久力", "マジックポイント", "ダメージボーナス",]
        self.entries = {}
    ...
    def setvalues(self, name):
        ...
        if status != None:
            for entry, c in zip(self.entries.values(), list(status["status"][0].values())):
                entry.insert(0, c)
        else:
